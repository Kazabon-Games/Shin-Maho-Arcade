...
| `catalysts` | array of catalyst-type IDs (`catalysts.length + talismans.length` ≤ Pack, a shared pool) | `[]` — no owned Catalysts yet |
| `talismans` | array of talisman-type IDs (same shared Pack pool as `catalysts`) | `[]` — no owned Talismans yet |
